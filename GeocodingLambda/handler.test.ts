...
describe("Geocoding lambda tests", () => {
  it("Should insert a placemark with the proper address with the given lat/lon", async () => {
    ...
    expect(result).toEqual(
      'Placemark at {"latitude":36.99813840222285,"longitude":-122.05564377465653} successfully inserted. Address is \
...
"unit_number":""}'
  ...
  it("Should return when a placemark already exists", async () => {
    ...
    expect(result).toEqual(
      'Placemark at {"latitude":36.99813840222285,"longitude":-122.05564377465653} already exists'
    )
  })
})
