...
export const joinEvent = (async ({
  context: { selfId },
  params: { eventId },
  body: { region: { coordinate } = { coordinate: undefined } } = {}
}) =>
  conn
    .transaction((tx) =>
      getEventSQL(tx, eventId, selfId)
        ...
        .mapFailure((result) =>
          result.status === 403
            ? resp(403, { error: result.data.error })
            : result
        )
        .mapSuccess(async (event) => {
          const [status, hasArrived = false, trackableRegions] =
            await Promise.all([
              ...
              coordinate && areCoordinatesEqual(coordinate, event)
                ? insertArrival(tx, selfId, coordinate)
                    .withSuccess(true)
                    .unwrap()
                : success().unwrap(),
    ...
    .unwrap()) satisfies TiFAPIRouterExtension["joinEvent"]
