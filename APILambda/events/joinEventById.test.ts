...
import { callBlockUser, createUserAndUpdateAuth } from "../test/helpers/users.js"
...
describe("Join the event by id tests", () => {
  ...
  it("should return 200 when the user is able to successfully join the event", async () => {
    const eventOwnerToken = await createUserAndUpdateAuth(global.defaultUser.auth)
    const attendeeToken = await createUserAndUpdateAuth(global.defaultUser2.auth)
  ...
  it("should return 403 when the event has ended", async () => {
    ...
    const testEvent = { ...testEvents[0], endTimestamp: pastDate }
    const event = await callCreateEvent(eventOwnerToken, testEvent)
    const resp = await callJoinEvent(attendeeToken, parseInt(event.body.id))
    expect(resp.status).toEqual(403)
  })
