...
describe("generateUniqueUsername", () => {
  ...
  it("should clean name properly", async () => {
    ...
    expect(JSON.stringify(username.value).substr(0, 6)).toEqual("jhnd0e")
    expect(JSON.stringify(username.value).substr(6)).toMatch(/^\d{4}$/)
  })
  ...
  it("should produce a handle starting with 'idk' if name has 0 valid characters", async () => {
    ...
    expect(JSON.stringify(username.value).substr(0, 3)).toEqual("idk")
    expect(JSON.stringify(username.value).substr(3)).toMatch(/^\d{4}$/)
  })
